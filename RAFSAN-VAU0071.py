...
⠀⠀                        ⠀  ⢀⣾⠟{K2}⢀⣼⣿⠟⠋⠀⠀⠀⠀⠀⠀⠀⠀⠉⠻⣿⣷⡄{H2}⠹⣷⡀⠀⠀⠀
                             ⣾⡏{K2}⢠⣿⣿⡯⠤⠤⠤⠒⠒⠒⠒⠒⠒⠒⠤⠤⠽⣿⣿⡆{H2}⠹⣷⡀⠀⠀
                  _    _    ⢸⣟{K2}⣠⡿⠿⠟⠒⣒⣒⣈⣉⣉⣉⣉⣉⣉⣉⣁⣒⣒⡛⠻⠿⢤{H2}⣹⣇                _
       /\  /\__ _| | _(_)   ⣾⡭{K2}⢤⣤⣠⡞⠉⠉{M2}⢀⣀⣀⠀⠀⠀⠀⢀⣀⣀⠀{K2}⠈⢹⣦⣤⡤{H2}⠴⣿     /\/\   ___| |_ __ _
      / /_/ / _` | |/ / |   ⣿⡇{K2}⢸⣿⣿⣇⠀{M2}⣼⣿⣿⣿⣷⠀⠀⣼⣿⣿⣿⣷⠀{K2}⢸⣿⣿⡇⠀{H2}⣿    /    \ / _ \ __/ _` |
     / __  / (_| |   <| |   ⢻⡇{K2}⠸⣿⣿⣿⡄{M2}⢿⣿⣿⣿⡿⠀⠀⢿⣿⣿⣿⡿⢀{K2}⣿⣿⣿⡇{H2}⢸⣿   / /\/\ \  __/ || (_| |⠀
     \/ /_/ \__,_|_|\_\_|   ⠸⣿⡀{K2}⢿⣿⣿⣿⣆{M2}⠉⠛⠋⠁⢴⣶⠀⠉⠛⠉{K2}⣠⣿⣿⣿⡿⠀{H2}⣾⠇   \/    \/\___|\__\__,_|
                             ⢻⣷⡈{K2}⢻⣿⣿⣿⣿⣶⣤⣀⣈⣁⣀⡤⣴⣿⣿⣿⣿⡿⠁{H2}⣼⠟
                           ⢀  ⣽⣷⣄{K2}⠙⢿⣿⣿⡟⢲⠧⡦⠼⠤⢷⢺⣿⣿⡿⠋{H2}⣠⣾⢿⣄
                          ⢰⠟⠛⠟⠁⣨⡿⢷⣤⣈{K2}⠙⢿⡙⠒⠓⠒⠓⠚⣹⠛{H2}⢉⣠⣾⠿⣧⡀⠙⠋⠙⣆
                          ⠹⣄⡀⠀⠐⡏⠀⠀⠉⠛⠿⣶⣿⣦⣤⣤⣤⣶⣷⡾⠟⠋⠀⠀⢸⡇⠀⢠⣤⠟
...
def xoshnaw():
  ...
  try:
    ...
    if id in httpCaht:
      ...
      whoami(f'''[bold red]LICENSI ANDA TIDAK RAFSAN [😡]''','color(8)')
      whoami(f'''[bold yellow]SILAHKAN COPY ID DI ATAS RAFSAN AUTHOR [📩]''','color(8)')
      whoami(f'''[bold green]Whatsapp[bold white] : [bold white] +8801608590758 [bold green][📲]''','color(8)')
      os.system('xdg-open https://wa.me/+8801608590758?text=BANG+SAYA+MAU+BELI+LICENSI+CRACK+FB+BERAPA+HARGA+NYA+?')
      time.sleep(1)
      sys.exit()
  except:
    sys.exit()
...
class Menu:
	...
	def menu(self):
		...
		self.jol = Console()
		self.tol = Console()
		prints(Panel(f"{K2}        {self.negara}",width=87,padding=(0,30),title=f"{M2}• {H2}• {K2}• {H2}Negara {M2}• {H2}• {K2}•",subtitle=f"{M2}• {H2}• {K2}• {H2}Version : BETA.9{M2} • {H2}• {K2}•",style=f"{color_panel}"))
...
class Crack:
	...
	def metode_api(self,email,pwx):
		prog.update(des,description=f" {H2}• {P2}[{H2}Luffy-XD{P2}] {P2}[{P2}{str(self.loop)}{P2}/{P2}{len(tampung)}{P2}]{P2} [OK : {H2}{len(self.ok)}{P2} CP : {K2}{len(self.cp)}{P2}] [")
		prog.advance(des)
		try:
			for pw in pwx:
				pw = pw.lower()
